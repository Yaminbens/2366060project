...
def data_generator(x_in, y_in, batch_size=128):
    while True:
        random_perm = np.array([np.random.permutation(tiles_per_dim ** 2) for _ in range(batch_size)])
        ...
        y = keras.utils.to_categorical(y_rnd_perm, tiles_per_dim ** 2)
...
model.save_weights('weights' + str(tiles_per_dim))
